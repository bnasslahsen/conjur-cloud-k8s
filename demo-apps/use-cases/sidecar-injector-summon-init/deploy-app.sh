...
# SUMMON CONFIGMAP
$KUBE_CLI delete configmap sidecar-injector-summon-config-init --ignore-not-found=true
envsubst < secrets.template.yml > secrets.yml
$KUBE_CLI create configmap sidecar-injector-summon-config-init --from-file=secrets.yml
rm secrets.yml

# ADMISSION WEBHOOK CONTROLLER CONNECT MAP
$KUBE_CLI delete configmap sidecar-injector-summon-init-connect --ignore-not-found=true
...
$KUBE_CLI create configmap sidecar-injector-summon-init-connect \
...
if ! $KUBE_CLI wait deployment "$APP_SIDECAR_INJECTOR_SUMMON_INIT" --for condition=Available=True --timeout=90s
  then exit 1
fi
